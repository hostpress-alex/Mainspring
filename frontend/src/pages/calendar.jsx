...
export function CalendarPage () {
    ...
    useEffect(() => {
        boardService.query()
            .then(boards => setTasks(scheduleService.tasksFromBoards(boards)))
            .catch(e => setErr(readErr(e)))
    }, [])

    useEffect(() => { localStorage.setItem('calView', view) }, [view])
        ...
        <div className='cal'>
            <div className='cal-topbar'>
                ...
                <div className='cal-topbar-right'>
                    ...
                    }}>+ Zeit einplanen</button>
                    <div className='cal-switch'>
                        {VIEWS.map(v => (
                            <button key={v.key} className={view === v.key ? 'is-active' : ''}
            ...
            {view === 'month' ? (
                <MonthGrid
                    ...
                    onCreate={setDraft}
                    onOpen={setDraft}
                    onPickDay={day => { setAnchor(startOfDay(day)); setView('day') }}
                />
            ) : (
                <TimeGrid
