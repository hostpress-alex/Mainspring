...
export function EntryDialog({draft, tasks, onSave, onDelete, onClose, busy}){
    ...
    return (
        <div className="cal-backdrop" onMouseDown={ev => {
            ...
            <form className="cal-dialog" onSubmit={submit}>
                ...
                <div className="cal-dialog-body">
                    ...
                    <div className="cal-field">
                        ...
                        <select id="cal-board" ref={elFirst} value={boardId} onChange={e => onChangeBoard(e.target.value)}>
                    ...
                    <div className="cal-field">
                        ...
                        <input id="cal-filter" value={filter} placeholder={t('calendar.filterPlaceholder')} disabled={!boardId} onChange={e => setFilter(e.target.value)}/>
                    </div>

                    <div className="cal-field">
                        <label htmlFor="cal-task">{t('task.task')}{boardId?` (${visibleTasks.length})`:''}</label>
                        <select id="cal-task" size={Math.min(Math.max(visibleTasks.length, 2), 7)} disabled={!boardId} value={taskKey} onChange={e => setTaskKey(e.target.value)}>
                            {!boardId && <option value="" disabled>{t('calendar.chooseBoardFirst')}</option>}
                            {boardId && !visibleTasks.length &&
                                <option value="" disabled>{t('calendar.noTaskFound')}</option>}
                    ...
                    <div className="cal-row2">
                        <div className="cal-field">
                            ...
                            <input id="cal-start" type="datetime-local" value={start} onChange={e => onChangeStart(e.target.value)}/>
                        </div>
                        <div className="cal-field">
                            <label htmlFor="cal-end">{t('calendar.end')}</label>
                            <input id="cal-end" type="datetime-local" value={end} onChange={e => setEnd(e.target.value)}/>
                        </div>
                    </div>
                    ...
                    <div className="cal-field">
                        ...
                        <div className="cal-presets">
                            {PRESETS.map(m => (
                                <button type="button" key={m} className={`cal-preset${Math.round(durationMs / MS_MIN) === m?' is-active':''}`} onClick={() => applyPreset(m)}>
                ...
                <div className="cal-dialog-foot">
                    <div>
                        {isEdit && (
                            <button type="button" className="cal-btn cal-btn-danger" disabled={busy} onClick={async() => {
