/**
 * Speicherzugriff auf Benutzer — MariaDB.
 * Gegenstueck: user.repo.mongo.js. Ausgewaehlt wird in user.repo.js.
 */
...
async function query(filterBy = {}) {
    ...
    if (filterBy.txt) {
        ...
        q = q.where(function () {
            this.whereRaw('LOWER(username) LIKE ?', ['%' + needle + '%'])
                .orWhereRaw('LOWER(fullname) LIKE ?', ['%' + needle + '%'])
        })
    }
